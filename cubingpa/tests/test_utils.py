...
def test_remove_not_progressing_solves_superior_in_the_middle():
...
def test_remove_not_progressing_solves_equal_in_the_middle():
    df_before = pd.DataFrame({'best': [50, 40, 40, 35]}, index=[0,1,2,3])
    df_expected = pd.DataFrame({'best': [50, 40, 35]}, index=[0,1,3])
    df_after = utils.remove_not_progressing_solves(df_before)
    assert df_expected.equals(df_after)

def test_remove_not_progressing_solves_superior_starting():
...
def test_remove_not_progressing_solves_superior_ending():
...
def test_interpolate_dates_interpolate_once():
...
def test_interpolate_dates_interpolate_multiple():
...
def test_interpolate_dates_interpolate_nothing():
...
def test_convert_date_index_to_timedelta_multiple_days():
    df_before = pd.DataFrame({'best': [50.0, 40.0, 30.0]}, index=pd.to_datetime(['01/01/2019','01/02/2019','01/03/2019']))
...
def test_convert_date_index_to_timedelta_one_day():
